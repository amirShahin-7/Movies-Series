...
const MobileSidebar = () => {
  const { details, externalIds } = useSelector((state) => state.movieDetails);
  const { movieKeywords } = useSelector((state) => state.keywordsMovies);
  ...
  return (
    <motion.div
      className="lg:hidden w-full bg-gradient-to-br from-[#8697C4]/60 via-[#ADBBDA]/60 to-[#8697C4]/60 backdrop-blur-md rounded-lg p-4 border-l border-l-[#3D52A0]/20 mt-4"
    ...
    >
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-col gap-3">
          <span className="text-sm text-[#3D52A0]/80 font-medium">Budget:</span>
          <span className="text-sm text-[#3D52A0] font-semibold">
            ${details?.budget || "0"}
          </span>
        </div>
