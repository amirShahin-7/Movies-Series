...
const Movies = () => {
  ...
  useEffect(() => {
    ...
    setPage(initialPage);
    dispatch(getMovies(initialPage));
  }, [location.search]);
              ...
              {movies.map((movie, idx) => (
                <SwiperSlide key={movie.id} className="w-[75%] mx-auto">
                  ...
                  >
                    ...
                    >
                      <div
                        className="relative bg-white/10 backdrop-blur-xl rounded-3xl overflow-hidden shadow-2xl transform transition-all duration-500 ring-0 will-change-transform [&:not(.swiper-slide-active)]:rotate-[-4deg] [&.swiper-slide-active]:scale-110 [&.swiper-slide-active]:ring-4 [&.swiper-slide-active]:ring-[#3D52A0]/40 [&.swiper-slide-active]:z-30"
                        style={{
                      ...
                      >
                        ...
                        <div className="p-6 flex flex-col h-[220px] relative z-20">
                          <h3 className="text-xl font-black mb-1 bg-clip-text text-transparent bg-gradient-to-r from-[#3D52A0] to-[#7091E6]">
                            {movie.title}
                          </h3>
                          <p className="text-[12px] italic mb-3 text-[#8697C4]/90 truncate">
                            {getGenres(movie.genre_ids)}
                          </p>
                          <p className="text-[13px] text-[#3D52A0]/80 mb-4 line-clamp-4">
                            {movie.overview}
                          </p>
            ...
            <div className="mt-6 flex justify-center flex-wrap gap-2">
              {movies.map((_, i) => (
                <button
                  key={i}
                  onClick={() => goTo(i)}
                  className="w-8 h-8 rounded-full flex items-center justify-center bg-white/30 text-[#3D52A0] font-bold shadow-md transition-all hover:bg-white/50"
                >
                  {i + 1}
                </button>
              ))}
