...
import { fetchKeywordSeries } from "./../../../redux/slices/series/keywords/keywordsSlice";

const KeywordPage = () => {
  const { keyword_id, keyword_name } = useParams();
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { keywordSeries, loading, error } = useSelector(
    (state) => state.keywordsReducer
  );
  const [page, setPage] = useState(1);

  useEffect(() => {
    dispatch(fetchKeywordSeries({ keywordId: keyword_id, page }));
    ...
    <div className="bg-gradient-to-br from-[#EDE8F5] via-[#ADBBDA] to-[#8697C4] min-h-screen py-8">
      <div className="container mx-auto px-4">
        ...
        {keywordSeries.length > 0 ? (
          ...
          <Typography variant="h6" className="text-[#3D52A0]">
            No series found for this keyword.
          </Typography>
        )}
        {keywordSeries.length > 0 && (
          <div className="flex justify-center gap-4 mt-6">
            ...
            <button
              ...
              disabled={keywordSeries.length < 20}
              className="px-4 py-2 text-[#3D52A0] bg-[#ADBBDA]/30 hover:bg-[#7091E6]/30 rounded-md disabled:opacity-50"
            >
              Next
            </button>
